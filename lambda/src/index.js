 ...
 * be specified in options.js or changed if advanced mode is turned on
 */
...
/**
 * Handles Apple Music - plays artist tracks or plays a radio station for the
 * current track
 */
...
function playlistHandler(roomValue, presetValue, skillName, response) {
   ...
   actOnCoordinator(options, skillPath, roomValue, function(error, responseBodyJson) {
      if (error) {
         ...
         if (responseBodyJson) {
            var jsonResponse = JSON.parse(responseBodyJson);
            if (jsonResponse.status === 'error') {
               var msg = 'Oh <phoneme alphabet="ipa" ph="ˈfʌk">fork</phoneme>. ' +
                  jsonResponse.error;
               response.tell({
                  type: EchoSonos.prototype.speechOutputType.SSML,
         ...
         actOnCoordinator(options, '/play', roomValue, function(error, responseBodyJson) {
            if (error) {
               ...
               var msg;
               if (skillName === 'playlist') {
                  msg = "Started playlist " + presetValue + " in " + roomValue;
               } else if (skillName === 'favorite') {
                  msg = "Started favorite " + presetValue + " in " + roomValue;
               }

               genericResponse(error, response, msg);
            }
         });
      }
   });
...
function actOnCoordinator(options, actionPath, room, onCompleteFun) {
   console.log('actOnCoordinator', room, actionPath);
   var callCoordinator = function(coordinatorRoomName) {
   ...
   var handleZonesResponse = function(error, responseJson) {
      if (!error) {
         ...
         roomCoordinators[room] = coordinatorRoomName;
         callCoordinator(coordinatorRoomName);
      }
      else {
         onCompleteFun(error);
      }
   ...
   if (roomCoordinators[room]) {
      ...
      console.log("using cached coordinator", coordinatorRoomName);
      callCoordinator(coordinatorRoomName);
   } else {
      options.path = '/zones';
...
function genericResponse(error, response, success) {
   ...
   if (error) {
      ...
      msg = 'Good <phoneme alphabet="ipa" ph="ˈʃɪt">sheet</phoneme>.';
   }
